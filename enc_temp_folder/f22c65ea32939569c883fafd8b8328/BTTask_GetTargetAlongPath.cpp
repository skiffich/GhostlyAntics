...
#include "BTTask_GetTargetAlongPath.h"
...
EBTNodeResult::Type UBTTask_GetTargetAlongPath::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	AVillageResident* Villager = Cast<AVillageResident>(AIController->GetPawn());

	if (!IsValid(Villager))
	{
		UE_LOG(LogTemp, Warning, TEXT("Villager is invalid"));
		FinishLatentTask(OwnerComp, EBTNodeResult::Failed);
		return EBTNodeResult::Failed;
	}
